...
function renderPage() {
  ...
  renderList(motivationalQuotes, "quoteList");
}
...
function updateWeeklyAverage(data) {
  ...
  let last7 = entries.filter(([date]) => {
    let d = new Date(date);
    let diff = Math.floor((today - d) / (1000 * 60 * 60 * 24));
    return diff <= 6; // last 7 days
  });

  if (last7.length === 0) return;

  let sum = last7.reduce((acc, [, score]) => acc + score, 0);
  let avg = Math.round(sum / last7.length);

  let el = document.getElementById("weeklyAvg");
  if (el) el.innerText = "📊 Weekly Avg: " + avg;
}